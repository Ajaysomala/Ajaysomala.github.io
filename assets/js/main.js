...
(function () {
  ...
  (function neuralNet() {
    ...
    function frame(now) {
      ...
      const dark = isDark();
      const nodeColor  = dark ? '155,89,245' : '124,58,237';
      const lineColor  = dark ? '155,89,245' : '124,58,237';
      const pulseColor = dark ? '212,168,67'  : '180,130,0';
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < CONNECT_DIST) {
            const alpha = (1 - dist/CONNECT_DIST) * (dark ? 0.12 : 0.08);
            ctx.beginPath();
      ...
      for (let i = pulses.length - 1; i >= 0; i--) {
        ...
        const alpha = Math.sin(p.t * Math.PI) * (dark ? 0.85 : 0.65);
        // Glow effect
      ...
      nodes.forEach(function(n) {
        ...
        ctx.fillStyle = `rgba(${nodeColor},${breathe * (dark ? 0.55 : 0.35)})`;
        ctx.fill();
        // Node glow ring
        ctx.beginPath();
